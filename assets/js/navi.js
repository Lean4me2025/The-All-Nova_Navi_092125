...
async function suggest(){
  ...
  selected.forEach(id => {
    (tree[id]||[]).forEach(role => suggestions.set(role,(suggestions.get(role)||0)+1));
  });
  const sorted = Array.from(suggestions.entries()).sort((a,b)=>b[1]-a[1]);
  const mount = document.getElementById('suggestions');
  if(sorted.length===0){ mount.innerHTML = '<p>No traits selected yet. Visit Nova, choose traits, then come back.</p>'; return; }
  ...
  sorted.forEach(([role,score])=>{
    html.push(`<div class="card">
      ...
      <div style="margin-top:10px">
        <button class="btn" onclick="alert('Resume builder coming next for ${role}.')">Build Resume</button>
